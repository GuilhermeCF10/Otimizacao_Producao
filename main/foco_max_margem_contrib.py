    ...
    for i in range(1, len(dados_produto) + 1):
        for comb in combinations_with_replacement(dados_produto.index, i):
            ...
            if tempo_total <= tempo_maximo_diario:
                ...
                if margem_total > 0:
                    combinacoes_validas.append((dia+1, produtos_combinados['Produto'].tolist(), margem_total, tempo_total))

    todas_combinacoes_validas.extend(combinacoes_validas)

    # Escolher a melhor combinação para o dia
    if combinacoes_validas:
        melhor_combinacao = max(combinacoes_validas, key=lambda x: x[2])
...
df_combinacoes_validas = pd.DataFrame(todas_combinacoes_validas, columns=['Dia', 'Produtos', 'Margem Total', 'Tempo Total'])
...
df_combinacoes_validas.to_excel('./max_margem_contrib/combinacoes_validas.xlsx', index=False)
df_escolhas_diarias.to_excel('./max_margem_contrib/escolhas_diarias.xlsx', index=False)

# Comparação das vendas em 2 semanas versus produção realizada
vendas_vs_producao = dados_produto[['Produto', 'Vendas para 2 semanas']]
...
vendas_vs_producao.to_excel('./max_margem_contrib/comparacao_vendas_producao.xlsx', index=False)
...
plt.grid(True)
plt.savefig('./max_margem_contrib/margem_contribuicao_diaria.png')
...
plt.legend()
plt.savefig('./max_margem_contrib/comparacao_vendas_producao.png')
...
for dia in range(1, dias_de_producao + 1):
    ...
    plt.ylabel('Margem de Contribuição')
    plt.grid(True)
    plt.savefig(f'./max_margem_contrib/combinacoes_validas_dia_{dia}.png')